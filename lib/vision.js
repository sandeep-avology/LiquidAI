...
async function callGemini({ apiKey, system, messages, maxTokens }) {
  // Quota is tracked per (key, model) so a 429 on key 1 doesn't block key 2
  const keyId       = apiKey.slice(-8);
  const keyedModels = GEMINI_MODELS.map(m => `${keyId}:${m}`);

  if (quota.allExhausted(keyedModels)) return { ok: false, error: 'api_429' };

  let lastError = 'api_503';

  for (let i = 0; i < GEMINI_MODELS.length; i++) {
    const model  = GEMINI_MODELS[i];
    const qKey   = keyedModels[i];

    if (quota.isExhausted(qKey)) continue;

    const result = await callGeminiModel({ apiKey, model, system, messages, maxTokens });
    if (result.ok) { quota.reset(qKey); return result; }
