...
function getVisionStatus() {
  ...
  const keyId    = key ? key.slice(-8) : '';
  const trackIds = keyId ? MODELS.map(m => `${keyId}:${m}`) : MODELS;
  return {
    visionEnabled:    apiKeyStatus === 'configured',
    apiKeyStatus,
    apiKeyConfigured: apiKeyStatus === 'configured',
    provider,
    quotaExhausted:   quota.allExhausted(trackIds),
    quotaResetsAt:    quota.resetsAt(trackIds),
  };
}
